...
- **安装崩溃**：下载新版 APK 完成后，本应弹出系统安装界面，但程序直接闪退。
- **启动闪退**：部分机型在启动时或进入检查更新时，因读取版本信息触发 `IllegalAccessError`。

### 根因

1.  **安全限制 (Android 7.0+)**：`DownloadManager` 在高版本系统中禁用了 `COLUMN_LOCAL_FILENAME` 字段，试图读取该字段会抛出 `SecurityException`。同时 `file://` URI 的直接暴露受限。
2.  **字段私有化**：`com.smartisan.updater.Version` 中的所有核心业务字段均为 `private`，导致注入代码跨包访问时崩溃。

### 修复方案

- **标准流程适配**：参考主题系统的成熟方案，弃用 `local_filename`，改用官方推荐的 `getUriForDownloadedFile(id)` 获取安装 URI。
- **授权补全**：在安装 Intent 中显式添加 `FLAG_GRANT_READ_URI_PERMISSION`，并注入 `StrictMode` 豁免代码，双重保证安装器能正确解析下载包。
- **字段全量公开**：将 `Version.smali` 中所有实例字段权限由 `private` 修改为 `public`，彻底杜绝权限类闪退。

### 结果

- **版本更新**：v1.5.4.5
- **体验提升**：更新系统逻辑已达到现代 Android 适配标准，安装弹窗感应顺滑，无任何残留权限报错。

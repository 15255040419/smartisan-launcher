...
- **[2026-04-19] v1.5.3.2 终极回归版**：完美适配 Android 16 底部布局，找回经典 UI 弹窗风格，正位仓库更新地址。
...
<<<<<<< HEAD
- `2026-04-14`：新增OTA在线更新，优化检查逻辑、静默提醒与弹窗视觉风格。
...
- `2026-04-12`：在线更新改为对接 GitHub Release；保留手动检查更新，同时把自动检查触发点调整为回到桌面时静默检查，每天最多一次。`v1.5.3.1.2` 为首个支持当前机制的版本，更早版本需先手动升级。
=======
>>>>>>> 097dc6c (fix(theme): 修复四指切主题卡住并更新说明)
...
- `v1.5.3.1.2` 是首个支持当前这套在线更新机制的版本；`r5` 之前发布的旧版没有接入当前 GitHub Release 检查链路，无法通过应用内“检查更新”直接升级到 `r5`。
- 如果当前设备安装的是 `r5` 之前的版本，需要先手动下载安装 `v1.5.3.1.2`；从 `r5` 开始，后续版本才可以继续通过应用内更新接收新版本。

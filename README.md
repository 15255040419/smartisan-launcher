...
- 修复针对 Android 12 ~ 16 因状态栏、手势导航和三键导航导致的桌面首页底部留缝问题
- 找回并重构了经典的锤子风格在线更新弹窗，告别原生黑框 Toast
- 天气入口适配：优先拉起系统或已安装的天气应用，解决旧接口失效问题
- 毛玻璃 / 白雾主题专属：支持在桌面设置中手动选择壁纸或恢复默认
- 独立生态：更新服务器、主题下载完全迁移至私有 GitHub 仓库，不再依赖第三方
- 自动化构建：集成 GitHub Actions，支持推送标签自动进行 APK 签名与 Release 发布
...
- **[2026-04-19] v1.5.3.2.2.2.2 终极回归版**：完美适配 Android 16 底部布局，找回经典 UI 弹窗风格，正位仓库更新地址。
...
- 在线更新改为对接 GitHub Release；手动检查保留在设置页，每天自动静默检查一次
- 检查到新版本时，会弹出符合锤子风格的精致对话框，显示详细更新内容
...
- `2026-04-19`：**v1.5.3.2.2.2.2 终极版**。完美适配 Android 16 底部留缝，回归锤子风格更新弹窗。
- `2026-04-11`：搜索页支持分身/双开应用展示与启动。
- `2026-04-10`：优化天气入口逻辑，优先拉起第三方天气 App。
- `2026-04-09`：修复默认桌面设置跳转无响应问题。
...
>>>>>>> 097dc6c (fix(theme): 修复四指切主题卡住并更新说明)
...
- `v1.5.3.2.2.2.1.2` 是首个支持当前这套在线更新机制的版本；`r5` 之前发布的旧版没有接入当前 GitHub Release 检查链路，无法通过应用内“检查更新”直接升级到 `r5`。
- 如果当前设备安装的是 `r5` 之前的版本，需要先手动下载安装 `v1.5.3.2.2.2.1.2`；从 `r5` 开始，后续版本才可以继续通过应用内更新接收新版本。
...
- 推荐直接前往 [Releases](https://github.com/15255040419/smartisan-launcher/releases) 页面下载已签名 APK
- 当前最新版本可在 [Latest Release](https://github.com/15255040419/smartisan-launcher/releases/latest) 获取

...
- `v1.5.3` 是首个支持当前 GitHub Release 在线更新机制的版本；`r5` 之前的旧版本需要先手动安装 `r5`，后续版本才可继续走应用内更新
=======
...
- `2026-04-12`：在线更新改为对接 GitHub Release；保留手动检查更新，同时把自动检查触发点调整为回到桌面时静默检查，每天最多一次。`v1.5.3` 为首个支持当前机制的版本，更早版本需先手动升级。
=======
>>>>>>> 097dc6c (fix(theme): 修复四指切主题卡住并更新说明)
...
- `v1.5.3` 是首个支持当前这套在线更新机制的版本；`r5` 之前发布的旧版没有接入当前 GitHub Release 检查链路，无法通过应用内“检查更新”直接升级到 `r5`。
- 如果当前设备安装的是 `r5` 之前的版本，需要先手动下载安装 `v1.5.3`；从 `r5` 开始，后续版本才可以继续通过应用内更新接收新版本。
